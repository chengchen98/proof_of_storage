...
pub struct MiMC5HashDemo<'a, F: Field> {
    x_inputs: &'a [Option<F>],
    key: Option<F>,
    constants: &'a [F],
}
...
impl<'a, F: Field> ConstraintSynthesizer<F> for MiMC5HashDemo<'a, F> {
    fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> Result<(), SynthesisError> {
        assert_eq!(self.constants.len(), MIMC5_HASH_ROUNDS);
...
fn test_mimc5_hash() {
    ...
    let constants = (0..MIMC5_HASH_ROUNDS).map(|_| rng.gen()).collect::<Vec<_>>();

    // Generate a random preimage and compute the image
    let x_hash = multi_mimc5_hash(&x_inputs, key, &constants);

    println!("Creating parameters...");

    // Create parameters for our circuit
    let params = {
        let c = MiMC5HashDemo::<Fr> {
